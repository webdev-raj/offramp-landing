...
function ActivityIcon({ type }) {
  ...
  return (
    <div className="w-10 h-10 rounded-full bg-[#F5AE38]/15 text-[#F5AE38] flex items-center justify-center shrink-0 border border-[#F5AE38]/30">
      <Zap className="w-5 h-5 fill-[#F5AE38]" />
    </div>
  );
}
  ...
  return (
    <div className="mb-8">
      ...
      <div className="flex items-center gap-2 mb-4">
        <span className="text-[#1B7042] font-bold text-xs">◆</span>
      ...
      {/* Activity List Card */}
      <div className="bg-white border-2 border-[#E8DCC4] rounded-3xl shadow-sm divide-y divide-[#E8DCC4]/50 overflow-hidden">
        {items.length === 0 ? (
          ...
          items.map((item) => (
            <div
              key={item.id}
              className="flex items-center justify-between gap-4 p-4 sm:p-5 hover:bg-[#FBF3E3]/40 transition-colors"
            >
              <div className="flex items-center gap-3.5 sm:gap-4 min-w-0">
                <ActivityIcon type={item.type} />
                <div className="min-w-0">
                  <p className="font-montserrat font-black text-sm sm:text-base text-[#1E2538] truncate">
                    {item.description}
                  </p>
                  <p className="font-dmsans text-xs text-[#9A9AAA] mt-0.5">
                    {item.timeAgo}
                  </p>
                </div>
              </div>
              ...
              <div className="shrink-0 text-right">
                <span className="font-montserrat font-black text-base sm:text-lg text-[#1B7042]">
                  +{item.points}
                </span>
              </div>
            </div>
